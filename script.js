(() => {
  ...
  function loadState() {
    try {
      ...
      s.bestAtGameStart = Number.isFinite(s.bestAtGameStart) ? s.bestAtGameStart : s.best;
      return s;
    } catch { return null; }
  }
  ...
  function newGame(keepBest=true){
    ...
    state.bestAtGameStart = best; // baseline for this run
  ...
  // ※ 게임 도중 최고기록을 갱신했더라도, 팝업은 게임오버 순간에 1번만 띄운다.
  function shouldAskNameOnGameOver(){
    const isNewBestThisRun = state.score > (state.bestAtGameStart ?? 0);
    const notNamedYet = state.bestNamedAt < state.best;
    return isNewBestThisRun && notNamedYet;
  }
  ...
  function afterMoveFinalize(prevBest){
    ...
    if (!Number.isFinite(state.bestAtGameStart)) state.bestAtGameStart = state.best;
  ...
  function restore(){
    ...
    if (state.status === "over") {
      const isNewBestThisRun = state.score > (state.bestAtGameStart ?? 0);
      const needName = isNewBestThisRun && (state.bestNamedAt < state.best);
      if (needName) {
        pendingGameOverAfterName = true;
        showHighScoreNamePopup();
      } else {
